...
v(t) = 4/π≤ft(sinω t + 1/3sin 3ω t + 1/5sin 5ω t + 1/7sin 7ω t + ·s)
...
在对数坐标系中，理想方波的高频段是一条斜线：**每增加十倍频程（频率 x 10），幅度下降 20dB（幅度 x 0.1）。**

-20dB/dec 这个斜率是本章最重要的基本量。
...
高频段以 -20dB/dec 斜率一直衰减下去。不管多高频，总有那么一点点能量。

### 梯形波的频谱：两个拐点

梯形波的频谱有两个关键频率拐点：

**第一拐点 f_1 = 1/π · t_p**（t_p 为脉冲宽度）

频率超过 f_1 之后，幅度开始以 -20dB/dec 下降。这和理想方波一样。

**第二拐点 f_2 = 1/π · T_r**

频率超过 f_2 之后，幅度以 **-40dB/dec** 加速下降。

- -20dB/dec = 频率 x 10，幅度 x 0.1
- -40dB/dec = 频率 x 10，幅度 x 0.01

衰减速度差了 10 倍。超过 f_2 后高频分量被迅速削掉。

### Tr 怎么决定一切

T_r出现在第二个拐点的分母上。T_r越小，f_2越大，加速衰减开始得越晚。
...
|:--|:--|:--|
| f_2 | ≈ 318MHz | ≈ 3.18GHz |
| < 318MHz | -20dB/dec 温和衰减 | -20dB/dec 温和衰减 |
| 318MHz ~ 3.18GHz | -40dB/dec **加速衰减** | -20dB/dec **仍在温和衰减** |
| > 3.18GHz | — | -40dB/dec 加速衰减 |

看 1GHz 处：

- T_r = 1ns 的信号在 1GHz 时已进入加速衰减段，幅度被压得很低。
- T_r = 0.1ns 的信号在 1GHz 时仍在温和衰减段，幅度还很可观。

> **T_r 减小 → 拐点 f_2 推高 → 更宽频率范围保留高频能量 → SI 问题更严重。** 这就是第一章 "Tr 减小是 SI 根源" 的数学证明。
...
BW = 0.5/T_r，心里过一下。T_r = 1ns → 500MHz；T_r = 0.3ns → 1.7GHz。
...
- 100MHz < BW < 500MHz → 看走线长度。走线延时 > T_r/6 就得端接。FR4 信号速度约 15cm/ns，T_r = 1ns 时临界长度 ≈ 2.5cm
- BW > 500MHz → 必须认真对待，控阻抗、端接、等长
...
- 探头带宽也要满足（被动探头通常 200~500MHz，得用有源探头）
- 采样率至少 5 x BW（Nyquist 大法好）
...
T_r → BW = k/T_r → 高频分量多少 → SI 问题程度

- 傅里叶分解：方波 = 一堆正弦波叠加
- 梯形波有两个拐点，f_2 = 1/(π · T_r) 由 Tr 决定
