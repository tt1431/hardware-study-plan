...
其中 $v_p 是信号在 PCB 上的传播速度。FR4 微带线：v_p \approx 15\text{ cm/ns}$（真空光速的约一半）。
...
在大多数数字 PCB 场景下，$R 和 G 远小于 \omega L 和 \omega C$（高频下感抗 >> 电阻），可以先当**无损传输线**处理：
...
**② 传播：** 电压/电流以速度 $v_p 沿走线向前推进。每前进一小段，都要给这一段的寄生电容充电、电感储能——这就是 Z_0 的物理感觉。$

**③ 到达：** 到达负载端。如果负载阻抗 ≠ $Z_0$，产生反射。

**④ 响应：** 反射波沿原路返回，叠加在入射信号上。往返一次的时间 = $2 \times T_d（T_d = 单向延迟）。$
...
负载阻抗 $Z_L 与特性阻抗 Z_0$ 不匹配时，产生反射。反射系数：
...
**读图方法：**
1. 第一条线：入射波 $V_1^+，从源端走到负载端，耗时 T_d$
2. 第二条线：$V_1^- = V_1^+ \times \Gamma_L，从负载端反射回源端，再耗时 T_d$
3. 第三条线：$V_2^+ = V_1^- \times \Gamma_S$，源端再次反射……

每往返一次（$2T_d，反射波幅度乘以 \Gamma_S \times \Gamma_L。如果 |\Gamma| < 1，幅度逐次衰减，最终趋于稳态。$
...
- 振铃周期：$T_{ring} = 4 \times T_d$（往返两次一个周期）
...
两个电阻：$R_1 上拉 VCC，R_2 下拉 GND。要求 R_1 \parallel R_2 = Z_0，且 V_{bias}$ 设定合适。
...
串联电容 $C + 并联电阻 R_t = Z_0$ 到地。电容隔直，只对反射产生的高频分量起作用。
