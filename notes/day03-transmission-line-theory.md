...
| 损耗角正切 $\tan\delta$ | 0.015 ~ 0.025 | 材料消耗电磁能量的程度，值越小损耗越低 |
| 玻璃化转变温度 Tg | 130°C ~ 180°C | 超过此温度材料软化，尺寸和电性能剧变 |
| 热膨胀系数 CTE | 12~16 ppm/°C (XY) | 温度变化时尺寸变化率，影响过孔可靠性 |

**为什么 FR4 无处不在？**

- ✅ 成本低 — 每平方米几十块钱，批量生产成本极 低
- ✅ 工艺成熟 — 全球 PCB 厂都对 FR4 了如指掌，良率 高
- ✅ 机械性能好 — 刚性够，钻孔、切割、焊接都容 易
- ✅ 阻燃 — 即使过热也不会持续燃 烧

**FR4 的局限（高速设计为什么嫌弃它）：**

- ❌ 介电常数不稳定 — $\epsilon_r$ 随频率和温度变化，GHz 以上波动明 显
- ❌ 损耗大 — $\tan\delta \approx 0.02$ 意味着 10GHz 信号每走 10cm 可能衰减 30% 以 上
- ❌ 吸湿 — 吸收空气中的水分后介电常数和损耗角都会增 大
...
> 💡 **记一个数：** FR4 微带线信号速度 ≈ 15 cm/ns（约光速的一半）。做高速设计时，这个数能帮你快速估算延迟和临界长度。
...
- 打开示波器，通道 1 接驱动端，通道 2 接接收 端
- 如果没有端接：接收端波形有明显过冲 + 振 铃
...
**分支/Stub：**
- 在主传输线上分叉出去一段开路走 线
- 开路 stub 等效为一个并联谐振回 路
- DDR 多颗粒的 Fly-by 拓扑本身就有一串 stub，靠端接来抑制反 射

**连接器：**
- 连接器的引脚电感 + 信号回流路径变 化
- SMA 连接器设计为 50Ω，但 PCB 上的过渡段需要仔细处 理
- 高速连接器（如 Samtec、Molex）内部做了阻抗控 制
...
> - Rs + Rout = $Z_0$ （驱动端阻抗匹配）
> - 信号以半幅发射 → 末端全反射叠到满幅 → 返回源端被吸 收
> - 仅适用于**单驱单收**的点对点拓 扑
> - 往返延迟内接收端处于中间电平，走线过长影响时 序

**② 末端并联匹配**

负载端并联 $R_t = Z_0$ 到地。

- ✅ 最大优点：彻底消除反射，任何拓扑都适 用
...
- 传播速度： $v_p = c/\sqrt{\epsilon_{\text{eff}}}$ ，比带状线 快

### 带状线 (Stripline)
- 走线在 PCB **内层**，上下各一个参考平 面
- 电磁场**全部**在 FR4 介质 中
...
- **定位不连续点**：连接器、过孔处的阻抗跳变在 TDR 上清清楚 楚
- **测差分阻抗**：TDR 有差分模式，可以直接测出 $Z_{diff}$ 。
- **检查电缆质量**：断点、压接不良处表现为阻抗尖 峰

### 3.8.3 读 TDR 波 形

```
阻抗↑ = 容性负载? 还是窄线? → 看波形形状判 断
阻抗↓ = 感性负载? 还是宽线? → 感性是尖峰，宽线是平 台
```
...
**关键参数：**
- L1 到 L2 的距离（介质厚度 H）决定顶层走线的特性阻 抗
- 这个距离在 PP 片选型和压合工艺时就确定了，不能事后 改
- 板厂根据你指定的阻抗和他们的材料参数，反算出线 宽
...
- **参考平面必须连续**：高速信号下方不能跨过电源层的分 割
- **换层时伴随参考平面**：L1 换到 L3（都参考 L2 GND）比 L1 换到 L4（参考平面变了）安 全
- **阻抗控制线走 50Ω 之前**，先确认你的板厂能不能做（一般都能，但公差 ±10% 要心里有数）
- **差分对间距**：告诉板厂你要 100Ω 差分，他们会告诉你线宽和线间距的 值
...
- 频率越高 → 有效截面积越小 → 等效电阻越 大
- PCB 铜箔表面粗糙度会加重趋肤损 耗

**介质损耗：**
- FR4 的环氧树脂在交变电场中消耗能 量
- 损耗角正切 $\tan\delta$ 越大，介质损耗越 大
...
```
Tr → BW（Day2）→ 高频分量在走线里的行为（Day3）→ 反射/端接/SI问 题
```
...
1. 临界长度判据： $L > T_r \times v_p / 6$ → 走线已是传输 线
2. 反射系数： $\Gamma = (Z_L - Z_0)/(Z_L + Z_0)$ 。
3. 源端串联：点对点场景最省事的方 案
4. 微带线 vs 带状线：表层快但有干扰，内层干净但 慢
5. 参考平面连续性：高速信号下方地平面不能 断
6. 阻抗交给板厂算：你定目标、定叠层 → 板厂給线 宽
